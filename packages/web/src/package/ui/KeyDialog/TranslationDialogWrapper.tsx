...
export const TranslationDialogWrapper = ({
  ...
  useEffect(() => {
    ...
    const handleCancel = (e: Event) => {
      // MUI's Dialog owns the Escape close flow; keep the browser from
      // closing the native dialog underneath it.
      e.preventDefault();
    };
  ...
  return (
    <>
      ...
      <dialog
        ref={nativeDialogRef}
        className={TOP_LAYER_CLASS}
        // MUI's Modal handles Escape but stops propagation without
        // preventDefault, so the browser's close request would fall through
        // to the host <dialog> once ours unmounts.
        onKeyDownCapture={(e) => {
